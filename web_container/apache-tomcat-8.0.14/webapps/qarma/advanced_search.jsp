...
String user_error_msg = ""; 
			...
			Integer[] cat_array = Request_utils.parse_categories_string(category, loc );
			...
			String converted_first_date = Utils.convert_value_to_date(first_date);
			...
			//split the users string on one or more whitespace or one or more non-word chars.
			String[] split_user_names = users.split("\\s+?|\\W+?"); 
			
			Integer[] user_ids = User_utils.get_user_ids_by_names(split_user_names);
			
			if (user_ids.length > 0 && users.length() > 0) {
				validation_error |= true;
